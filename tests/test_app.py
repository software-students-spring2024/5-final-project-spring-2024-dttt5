...
def test_calories(client):
    ...
    response = client.post('/calories', data=data, follow_redirects=True)
    assert response.status_code == 200
    assert response.request.path == url_for('index')

    # Test when user is logged in and making a GET request
    response = client.get('/calories')
    assert response.status_code == 200
